...
class TCoordSet
    ...
    /**
    Creates a coordinate set, specifying all parameters.
    aX points to the first X coordinate.
    aY points to the first Y coordinate.
    aStride is the distance in bytes between successive X or Y coordinates.
    ...
    */
    ...
    /** Creates a coordinate set from a vector of TPointFP. */
...
class TWritableCoordSet: public TCoordSet
    ...
    /** Creates a writable coordinate set with specified coordinates and stride. */
    TWritableCoordSet(double* aX,double* aY,size_t aStride,size_t aCount):
        TCoordSet(aX,aY,aStride,aCount)
        {
        }
    /** Creates a writable coordinate set referring to an array of points. */
